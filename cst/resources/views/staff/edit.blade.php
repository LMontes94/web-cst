                    ...
                    <form method="POST" action="{{ route('staff.update', $staff->id) }}">
                        ...
                        <div class="shadow overflow-hidden sm:rounded-md">
                            <div class="px-4 py-5 bg-white sm:p-6">
                                ...
                                <input type="text" name="name" id="name" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('name', $staff->name) }}" />
                                @error('name')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                                @enderror

                                <label for="email" class="block font-medium text-sm text-gray-700">Email</label>
                                <input type="email" name="email" id="email" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('email', $staff->email) }}" />
                                @error('email')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                ...
                                <select name="positions[]" id="positions" multiple class="form-multiselect block w-full mt-1">
                                    ...
                                    <option value="{{ $position->id }}" {{ $staff->positions->contains($position->id) ? 'selected' : '' }}>{{ $position->name }}</option>
                                    @endforeach
                                </select>
                                @error('positions')
                                <p class="text-sm text-red-600">{{ $message }}</p>
